...
import { createAiReviewer, resolveAiReviewConfig, } from './ai-review.js';
...
function normalizeSearchText(value, sensitive) {
    if (sensitive)
        return value;
...
export async function startGuitoServer({ repositoryPath, uiRoot, host, port = 8080, apiToken, azureDevOpsUrl, prBranchNameTemplate, autoReload, diffViewer, showGraph, showStashes, showTags, showRemoteBranches, issueRegex, issueUrl, allowMerge, fileListView, refListView, sidePanelSectionsExpanded, searchMode, searchCaseSensitive, aiReview, azureRequestImpl, aiReviewModelImpl, avatarFetchImpl, onLog, }) {
    ...
    const strField = (value) => (typeof value === 'string' ? value.trim() : '');
    /** Reads a boolean body field; only an explicit true enables the option. */
    const boolField = (value) => value === true;
    /** Sends `{ success: true }`, adding warnings for partially-failed composites. */
    const mutationResult = (resp, warnings) => resp.type('application/json').send({
        success: true,
    ...
    const mainlineArg = (mainline, parents) => {
    ...
    const azureMergeStatus = (pr) => {
        ...
        return ['notSet', 'queued', 'conflicts', 'succeeded', 'rejectedByPolicy', 'failure'].includes(raw)
            ? raw
            : 'notSet';
    };
    ...
    const effectiveSettings = async () => {
        ...
        return {
            ...
            prBranchNameTemplate: prBranchNameTemplate?.trim() || filePrBranchNameTemplate || DEFAULT_PR_BRANCH_NAME_TEMPLATE,
            source: azure.source,
            ...
            aiReview: resolveAiReviewConfig(file.aiReview, aiReview),
            issueLinking,
        };
    };
    ...
    app.post('/api/revert', async (req, resp) => {
        try {
            ...
            await mutate(async () => {
                ...
                if (parents.length > 1)
                    args.push('--mainline', mainlineArg(req.body?.mainline, parents));
                if (noCommit)
                    args.push('--no-commit');
                if (signoff)
                    args.push('--signoff');
    ...
    app.post('/api/commit/drop', async (req, resp) => {
        try {
            ...
            await mutate(async () => {
                ...
                const parents = await commitParents(hash);
                if (parents.length === 0)
                    throw new Error('The root commit cannot be dropped.');
                if (parents.length > 1)
            ...
            await mutate(async () => {
                ...
                if (publishRemote) {
                    try {
                        ...
                        warnings.push(`Branch "${name}" was created but could not be published to "${publishRemote}": ${err.message}`);
                    }
                }
            });
            ...
            await mutate(async () => {
                ...
                if (deleteRemote) {
                    try {
                        ...
                        warnings.push(`Branch "${name}" was deleted locally but could not be deleted on "${deleteRemote}": ${err.message}`);
                    }
                }
            });
            return mutationResult(resp, warnings);
    ...
    app.post('/api/branch/delete-remote', async (req, resp) => {
        try {
            ...
            if (!remote || !branch)
                throw new Error('A remote and branch are required.');
    ...
    app.post('/api/branch/rename', async (req, resp) => {
        try {
            ...
            if (!oldName || !newName)
                throw new Error('The current and new branch names are required.');
                ...
                if (remote) {
                    if (publish) {
                        try {
                            ...
                            warnings.push(`Branch "${newName}" was renamed but could not be published to "${remote}": ${err.message}`);
                        }
                    }
                    if (deleteOld) {
                        try {
                            ...
                            warnings.push(`The old branch "${oldName}" was renamed locally but could not be deleted on "${remote}": ${err.message}`);
                        }
                    }
                }
            });
    ...
    app.post('/api/checkout', async (req, resp) => {
        try {
            ...
            await mutate(async () => {
                // Resolve the remote up front so a bad configuration fails before the
                // checkout happens.
                const publishRemote = newBranch && boolField(req.body?.publish)
                    ? await resolveRemote(strField(req.body?.remote))
                    : '';
                if (newBranch) {
                ...
                if (publishRemote) {
                    try {
                        ...
                        warnings.push(`Branch "${newBranch}" was checked out but could not be published to "${publishRemote}": ${err.message}`);
                    }
                }
            });
            return mutationResult(resp, warnings);
    ...
    app.post('/api/merge', async (req, resp) => {
        try {
            const branch = strField(req.body?.branch);
            if (!branch)
                throw new Error('A branch or commit is required.');
            ...
            if (noCommit)
                args.push('--no-commit');
            if (boolField(req.body?.autostash))
                args.push('--autostash');
            args.push(branch);
    ...
    app.post('/api/stash/save', async (req, resp) => {
        try {
            ...
            const includeUntracked = typeof req.body?.includeUntracked === 'boolean' ? req.body.includeUntracked : scope !== 'staged';
            const args = ['push'];
            if (scope === 'staged')
                args.push('--staged');
            if (scope === 'unstaged')
    ...
    app.post('/api/stash/apply', async (req, resp) => {
        try {
            ...
            if (!Number.isInteger(index) || index < 0)
                throw new Error('A stash index is required.');
    ...
    app.post('/api/tag/create', async (req, resp) => {
        try {
            ...
            if (annotate && !message)
                throw new Error('An annotated tag requires a message.');
            ...
            await mutate(async () => {
                ...
                if (pushRemote) {
                    try {
                        ...
                        warnings.push(`Tag "${name}" was created but could not be pushed to "${pushRemote}": ${err.message}`);
                    }
                }
            });
            return mutationResult(resp, warnings);
            ...
            await mutate(async () => {
                ...
                if (deleteRemote) {
                    try {
                        ...
                        warnings.push(`Tag "${name}" was deleted locally but could not be deleted on "${deleteRemote}": ${err.message}`);
                    }
                }
            });
            return mutationResult(resp, warnings);
    ...
    app.post('/api/tag/push', async (req, resp) => {
        try {
            ...
            await mutate(() => git.push([remote, ...(boolField(req.body?.force) ? ['-f'] : []), `refs/tags/${name}`]));
            return mutationResult(resp, []);
        }
        catch (err) {
            return resp.status(400).type('application/json').send({ error: err.message });
    ...
    app.post('/api/pull', async (req, resp) => {
        try {
            ...
            await mutate(() => git.pull(resolvedRemote, branch || undefined, {
                ...(rebase ? { '--rebase': null } : {}),
                ...(ffOnly ? { '--ff-only': null } : {}),
            }));
            return mutationResult(resp, []);
    ...
    app.post('/api/settings', async (req, resp) => {
        try {
            ...
            if (body.aiReview && typeof body.aiReview === 'object') {
                ...
                settings.aiReview = resolveAiReviewConfig(settings.aiReview, body.aiReview);
            }
    ...
    const azurePrList = async (status) => {
        {
            const { prefix, remoteUrl } = await azureProjectContext();
            const repo = parseAzureRemoteUrl(remoteUrl).repo;
            ...
            return [...merged.values()]
                ...
                isReviewer: (Array.isArray(request?.reviewers) ? request.reviewers : []).some((entry) => String(entry?.id ?? '') === me.id),
    ...
    const azurePrDetail = async (id) => {
        {
            ...
            return {
                ...
                isReviewer: (Array.isArray(pr?.reviewers) ? pr.reviewers : []).some((entry) => String(entry?.id ?? '') === me.id),
     ...
     */
    const azurePrComment = async (id, body) => {
        {
            ...
            const endLine = Number.isInteger(requestedEndLine) && requestedEndLine >= line ? requestedEndLine : line;
            ...
            const created = await azureJson('POST', `${base}/threads?api-version=${AZURE_API_VERSION}`, JSON.stringify(payload));
     ...
     */
    const azurePrChangedFiles = async (id, sinceCommit = '') => {
        {
            const { base } = await azurePrApiBase(id);
            ...
            const baseIteration = sinceCommit
                ? list.find((entry) => String(entry?.sourceRefCommit?.commitId ?? '') === sinceCommit)
                : null;
            const compareTo = baseIteration && Number(baseIteration.id) !== Number(latest.id)
            ...
            let skip = 0;
            for (;;) {
                const data = await azureJson('GET', `${base}/iterations/${latest.id}/changes?$top=1000&$skip=${skip}${compareTo}` +
    ...
    app.get('/api/azure-devops/pullrequests/:id/changes', async (req, resp) => {
        try {
            const files = await azurePrChangedFiles(Number(req.params?.id), String(req.query?.since ?? ''));
    ...
    app.post('/api/azure-devops/pullrequests/:id/workitems', async (req, resp) => {
        try {
            ...
            const alreadyLinked = (await azureWorkItemRelations(prefix, workItemId)).some((relation) => String(relation?.url ?? '').toLowerCase() === artifactUrl.toLowerCase());
            if (!alreadyLinked) {
    ...
    app.delete('/api/azure-devops/pullrequests/:id/workitems/:workItemId', async (req, resp) => {
        try {
            ...
            const index = relations.findIndex((relation) => String(relation?.url ?? '').toLowerCase() === artifactUrl.toLowerCase());
            if (index >= 0) {
                await azurePatchWorkItemRelations(prefix, workItemId, [
                    {
                        op: 'test',
    ...
    app.post('/api/azure-devops/pullrequests/:id/checks/:evaluationId/requeue', async (req, resp) => {
        try {
            ...
            await azureJson('POST', `${prefix}/_apis/policy/evaluations/${encodeURIComponent(evaluationId)}` +
    ...
    const azurePrFileDiff = async (id, query) => {
        {
            ...
            const status = String(query?.changeType ?? '') ||
    ...
    const aiReviewer = createAiReviewer({
        listPullRequests: async () => (await azurePrList('active')).map((pr) => ({
            id: pr.id,
            title: pr.title,
        ...
        loadThreads: async (id) => (await azurePrThreads(id)).map((thread) => ({
            id: thread.id,
        ...
        writeStore: (store) => writeFile(reviewStorePath, JSON.stringify(store, null, 2) + '\n', 'utf8'),
        runModel: aiReviewModelImpl,
        log: (line) => onLog?.(line),
    }, resolveAiReviewConfig(aiReview), repositoryPath);
    /** Re-reads the settings file so file-level review settings take effect. */
    const refreshAiReviewConfig = async () => {
        const file = await readSettings();
        const config = resolveAiReviewConfig(file.aiReview, aiReview);
    ...
    app.get('/api/azure-devops/pullrequests/:id/ai-review', async (req, resp) => {
        try {
    ...
    app.post('/api/azure-devops/pullrequests/:id/ai-review/post', async (req, resp) => {
        try {
            const findingIds = (Array.isArray(req.body?.findingIds) ? req.body.findingIds : []).map((id) => String(id));
    ...
    app.post('/api/azure-devops/pullrequests/:id/ai-review/dismiss', async (req, resp) => {
        try {
            const findingIds = (Array.isArray(req.body?.findingIds) ? req.body.findingIds : []).map((id) => String(id));
    ...
    app.delete('/api/azure-devops/pullrequests/:id/ai-review', async (req, resp) => {
        try {
    ...
    app.get('/api/conflict', async (req, resp) => {
        try {
            await mutationQueue;
    ...
    app.post('/api/conflict/resolve', async (req, resp) => {
        try {
