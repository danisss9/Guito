...
export async function startGuitoServer({
  ...
  const azureProjectContext = async (): Promise<{
    prefix: string;
    remoteUrl: string;
  }> => {
  ...
  app.post("/api/azure-devops/pullrequest", async (req: any, resp) => {
    try {
      ...
      if (payloadWorkItems.length) {
        payload.workItemRefs = payloadWorkItems.map((id: number) => ({
          id: String(id),
        }));
      }
    ...
    try {
      ...
      if (!Array.isArray(pr?.labels)) {
        ...
        if (labelsResult.status < 200 || labelsResult.status >= 300) {
          throw new Error(
            azureErrorMessage(labelsResult) ||
  ...
  /** One normalized merge gate returned by the checks endpoint. */
  type AzureCheck = {
    id: string;
    name: string;
  ...
  const fetchPolicyEvaluations = async (
    prefix: string,
    projectId: string,
    prId: number,
  ): Promise<any[]> => {
