...
/** Identity of a user as returned by the Azure DevOps connection data. */
...
const AZURE_THREAD_STATUSES: Record<string, number> = {
  unknown: 0,
  active: 1,
  wontFix: 3,
  fixed: 4,
...
export async function startGuitoServer({
  ...
  const azureProjectContext = async (): Promise<{ prefix: string; remoteUrl: string }> => {
  ...
  let cachedAzureMe: AzureIdentity | null = null;
  const azureMe = async (): Promise<AzureIdentity> => {
    if (cachedAzureMe) {
  ...
  const azurePrApiBase = async (
    id: number,
  ): Promise<{ base: string; prefix: string; remoteUrl: string; repo: string }> => {
  ...
  app.post('/api/azure-devops/pullrequest', async (req: any, resp) => {
    try {
      ...
      const payloadReviewers = reviewerList
        .map((entry: any) => ({ id: String(entry?.id ?? ''), isRequired: !!entry?.required }))
  ...
  app.get('/api/azure-devops/pullrequests', async (req: any, resp) => {
    try {
      ...
      const collect = async (criteria: string): Promise<Map<number, any>> => {
  ...
  app.post('/api/azure-devops/pullrequests/:id/reviewers', async (req: any, resp) => {
    try {
      ...
      const payload: Record<string, unknown> = { isRequired: req.body?.required === true };
  ...
  app.post('/api/azure-devops/pullrequests/:id/autocomplete', async (req: any, resp) => {
    try {
      ...
      const buildPayload = (identity: Record<string, unknown> | null) => {
        const payload: Record<string, unknown> = { autoCompleteSetBy: identity };
        if (enabled) {
  ...
  app.post('/api/azure-devops/pullrequests/:id/threads/:threadId/status', async (req: any, resp) => {
    try {
  ...
  app.get('/api/azure-devops/pullrequests/:id/changes', async (req: any, resp) => {
    try {
      ...
      const data = await azureJson<any>(
        'GET',
        `${base}/iterations/${latest.id}/changes?api-version=${AZURE_API_VERSION}`,
      );
      ...
      const files = (Array.isArray(data?.value) ? data.value : []).map((entry: any) => {
