...
const file = path => ({ path, oldPath: path, status: 'modified', lines: [], additions: 1, deletions: 1 });
function history(count) {
  return Array.from({ length: count }, (_, i) => ({
    hash: (count - i).toString(16).padStart(40, '0'), message: `Commit ${i}`, date: '2026-09-07T12:00:00Z',
    author_name: i === 7 ? 'Special Author' : 'Test Author', author_email: 'test@example.test',
    parents: i < count - 1 ? [(count - i - 1).toString(16).padStart(40, '0')] : [], refs: i === 0 ? 'HEAD -> main' : '',
  }));
}
...
async function setup(page, count = 700) {
  ...
  const state = {
    staged: ['partial.txt'], unstaged: ['a.txt', 'b.txt', 'c.txt', 'd.txt', 'partial.txt'],
    failedStatus: false, failedSearch: false, failedHistory: false, failedCommit: false,
    delay: 0, searchDelay: 0, refreshDelay: 0, historyRequests: 0, stageRequests: [], commitRequests: [], contentRequests: [], conflicts: [],
  };
  const snapshot = () => ({
    files: [...new Set([...state.staged, ...state.unstaged])].map(file),
    staged: state.staged, unstaged: state.unstaged, untracked: [], conflicted: state.conflicts,
    stagedFiles: state.staged.map(file), unstagedFiles: state.unstaged.map(file),
  });
  await page.route('**/api/**', async route => {
    const request = route.request();
    expect(request.headers()['x-guito-token']).toBe('browser-test-token');
    const parsed = new URL(request.url()); const body = request.method() === 'POST' ? request.postDataJSON() : {};
    const send = (json, status = 200) => route.fulfill({ status, json });
    const delay = ms => new Promise(resolve => setTimeout(resolve, ms));
    switch (parsed.pathname) {
      case '/api/repo': return send({ name: 'Fixture', root: '/fixture' });
      case '/api/branches/all': return send([{ name: 'main', commit: commits[0].hash, current: true, remote: false }]);
      case '/api/commits': {
        ...
        return send({ commits: commits.slice(skip, skip + Number(parsed.searchParams.get('limit') || commits.length)), total: commits.length });
      }
      case '/api/commits/search': {
        const query = parsed.searchParams.get('query');
        const fail = state.failedSearch;
        await delay(state.searchDelay);
        return fail ? send({ error: 'Search unavailable' }, 400) : send({ hashes: query === 'body-only' ? [commits[600].hash] : [] });
      }
      case '/api/working-changes': return state.failedStatus ? send({ error: 'Status unavailable' }, 400) : send(snapshot());
      case '/api/stage': case '/api/unstage': {
      ...
      case '/api/commit': {
        state.commitRequests.push(body); await delay(state.delay);
        if (state.failedCommit) return send({ error: 'Commit hook rejected changes' }, 400);
        state.staged = []; return send({ success: true });
      }
      case '/api/file-content': state.contentRequests.push(body); return send({ content: `${body.ref}\n` });
      case '/api/commit/detail': return send({ ...commits.find(c => c.hash === body.hash), body: 'Details' });
      case '/api/commit/diff': return send({ hash: body.hash, files: [] });
      default: return send({ error: `Unexpected API: ${parsed.pathname}` }, 400);
    }
  });
...
test('header, working row and graph follow horizontal scrolling and column resize', async ({ page }) => {
  const { errors } = await setup(page);
  await page.locator('.working-row').click();
  await page.setViewportSize({ width: 1000, height: 700 });
  const viewport = page.locator('.list-viewport');
  await viewport.evaluate(el => { el.scrollLeft = 170; el.scrollTop = 2000; });
  await expect.poll(() => page.locator('.table-head').evaluate(el => getComputedStyle(el).transform)).not.toBe('matrix(1, 0, 0, 1, 0, 0)');
  const positions = await page.evaluate(() => {
    const x = selector => document.querySelector(selector).getBoundingClientRect().x;
    return [x('.table-head .cell-date'), x('.working-row .cell-date'), x('.list-viewport .row .cell-date')];
  });
  expect(Math.max(...positions) - Math.min(...positions)).toBeLessThan(1);
  expect(await page.locator('.graph-overlay').evaluate(el => getComputedStyle(el).transform)).toBe(await page.locator('.table-head').evaluate(el => getComputedStyle(el).transform));
  ...
  await page.mouse.move(box.x + 2, box.y + 10); await page.mouse.down(); await page.mouse.move(box.x + 82, box.y + 10); await page.mouse.up();
  await expect.poll(() => page.locator('.head-cell.cell-author').evaluate(el => el.getBoundingClientRect().width)).toBeGreaterThan(210);
  expect(errors).toEqual([]);
});

test('search after deep scroll, empty results, body matches and clear restore rows', async ({ page }) => {
  const { state, errors } = await setup(page);
  await page.locator('.list-viewport').evaluate(el => { el.scrollTop = 14000; });
  ...
  await expect(page.locator('.list-viewport .row')).toHaveCount(1);
  await expect(page.locator('.list-viewport .row')).toContainText('Commit 600');
  await search.fill('no-match'); await expect(page.getByText('No matching commits', { exact: true })).toBeVisible();
  await search.fill(''); await expect(page.locator('.list-viewport .row').first()).toContainText('Commit 0');
  await search.fill('Special Author'); await expect(page.locator('.list-viewport .row')).toHaveCount(1);
  await expect(page.locator('.list-viewport .row')).toContainText('Commit 7');
  expect(errors).toEqual([]);
});

test('rapid queries and refresh retain rows until latest search completes', async ({ page }) => {
  const { state } = await setup(page);
  state.searchDelay = 1000;
  const search = page.getByPlaceholder('Search commits');
  await search.fill('body-only'); await expect(page.locator('.table-loading')).toBeVisible();
  await search.fill('Commit 42');
  await page.getByTitle('Refresh').click();
  await expect(page.locator('.list-viewport .row').first()).toBeVisible();
  await expect(page.locator('.table-loading')).toHaveCount(0);
  await expect(page.locator('.list-viewport .row').first()).toContainText('Commit 42');
  await expect(page.locator('.list-viewport .row')).toHaveCount(11);
});
...
test('worker failure completes graph computation above 2000 rows', async ({ page }) => {
  await page.addInitScript(() => {
    window.Worker = class {
      postMessage() { setTimeout(() => this.onerror?.(new Event('error')), 50); }
      terminate() {}
    };
  });
  ...
  await expect(page.locator('.table-loading')).toHaveCount(0);
  await page.locator('.list-viewport').evaluate(el => { el.scrollTop = el.scrollHeight; });
...
test('modifier selection, separate diffs, bulk actions and drafts', async ({ page }) => {
  ...
  await unstaged.getByRole('option', { name: 'c.txt', exact: true }).click({ modifiers: ['Shift'] });
  await expect(unstaged.getByRole('option', { selected: true })).toHaveCount(3);
  await unstaged.getByRole('option', { name: 'b.txt', exact: true }).click({ modifiers: ['Control'] });
  ...
  await page.getByRole('button', { name: 'View unstaged diff for partial.txt', exact: true }).click();
  ...
  await page.getByRole('button', { name: 'Close changes' }).click(); await page.locator('.working-row').click();
...
test('failed status and commits preserve data; duplicate commits are prevented', async ({ page }) => {
  const { state } = await setup(page);
  await page.locator('.working-row').click();
  ...
  await expect(commit).toBeDisabled();
  state.failedStatus = false; await page.getByRole('button', { name: 'Retry status' }).click();
  await expect(commit).toBeEnabled();
  state.failedCommit = true; state.delay = 500;
  await commit.evaluate(el => { el.click(); el.click(); });
  await expect(commit).toBeDisabled();
  ...
  state.failedCommit = false; await expect(commit).toBeEnabled(); await commit.click();
...
test('authenticated iframe loads real history and working changes', async ({ page, request }) => {
  ...
  await page.goto(url);
  await page.setContent('<iframe title="Guito" style="width:100%;height:700px" src="/?guitoToken=browser-test-token"></iframe>');
...
test('search errors keep rows and retry; refresh loading respects reduced motion', async ({ page }) => {
  const { state } = await setup(page);
...
test('real graph worker renders large history and keyboard/modifier anchors are scoped', async ({ page }) => {
  ...
  await unstaged.getByRole('option', { name: 'partial.txt', exact: true }).click({ modifiers: ['Control', 'Shift'] });
  await expect(unstaged.getByRole('option', { selected: true })).toHaveCount(4);
  await staged.getByRole('option', { name: 'partial.txt', exact: true }).click({ modifiers: ['Shift'] });
  ...
  await page.keyboard.press('Space'); await page.keyboard.press('Shift+ArrowDown');
  await expect(unstaged.getByRole('option', { selected: true })).toHaveCount(2);
});

test('unresolved conflicts disable commit even with staged files and a message', async ({ page }) => {
  const { state } = await setup(page);
