...
const prRecord = (id, extra = {}) => ({
  ...
  createdBy: { id: ME.id, displayName: ME.displayName, uniqueName: ME.uniqueName },
...
test('lists my pull requests with votes, drafts, and required reviewers', async (context) => {
  const mine = prRecord(101, {
    title: 'Draft feature',
    isDraft: true,
    reviewers: [{ id: 'other-guid', displayName: 'Other', uniqueName: 'o@e.test', vote: 5 }],
  });
  const assigned = prRecord(202, {
    title: 'Needs my vote',
    creationDate: '2026-09-11T10:00:00.000Z',
    createdBy: { id: 'other-guid', displayName: 'Other', uniqueName: 'o@e.test' },
    reviewers: [
      { id: ME.id, displayName: ME.displayName, uniqueName: ME.uniqueName, vote: 10, isRequired: true },
      { id: 'other-guid', displayName: 'Other', uniqueName: 'o@e.test', vote: -10 },
    ],
  });
...
test('serves PR details and edits title, description, and draft state', async (context) => {
  ...
  const editResponse = await fetch(`${server.address}/api/azure-devops/pullrequests/101`, {
    ...
    body: JSON.stringify({ title: 'New title', description: 'New body', isDraft: true }),
  });
  assert.equal(editResponse.status, 200);
  const patch = calls.find((call) => call.method === 'PATCH');
  assert.deepEqual(patch.payload, { title: 'New title', description: 'New body', isDraft: true });
});
...
test('votes, adds required reviewers, and removes reviewers', async (context) => {
  ...
  assert.equal(
    calls[1].url,
    'https://azure.example/DefaultCollection/Project/_apis/git/repositories/Repo/pullrequests/202/reviewers/me-guid?api-version=5.0',
  );
...
test('completes pull requests and manages auto-complete', async (context) => {
  ...
  const cancelCalls = calls.filter(
    (call) =>
      call.payload && 'autoCompleteSetBy' in call.payload && call.payload.autoCompleteSetBy?.id !== ME.id,
  );
...
test('serves comment threads and posts replies, inline comments, and statuses', async (context) => {
  ...
  const inline = await fetch(`${base}/threads`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ content: 'Line note', filePath: 'src/app.ts', line: 12 }),
  });
      ...
      return {
        ...
        body: {
          value: [
            ...
            { changeType: 'rename', item: { path: '/renamed.ts' }, sourceServerItem: '/old.ts' },
